...
class B2PolygonShape extends b2Shape_2.B2Shape {
    ...
    Set(vertices, count = vertices.length, start = 0) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            for (let /*int32*/ j = 0; j < tempCount; ++j) {
                if (b2Math_1.B2Vec2.DistanceSquaredVV(v, ps[j]) <
                    0.5 * b2Settings_1.B2_linearSlop * (0.5 * b2Settings_1.B2_linearSlop)) {
                    unique = false;
                    break;
                }
            }
    ...
    ComputeMass(massData, density) {
        ...
        massData.I +=
            massData.mass *
                (b2Math_1.B2Vec2.DotVV(massData.center, massData.center) -
                    b2Math_1.B2Vec2.DotVV(center, center));
    }
    Validate() {
        for (let i = 0; i < this.m_count; ++i) {
            const i1 = i;
    ...
    ComputeSubmergedArea(normal, offset, xf, c) {
        ...
        const intoVec = B2PolygonShape.ComputeSubmergedArea_s_intoVec.Set(this.m_vertices[intoIndex].x * (1 - intoLamdda) +
            this.m_vertices[intoIndex2].x * intoLamdda, this.m_vertices[intoIndex].y * (1 - intoLamdda) +
            this.m_vertices[intoIndex2].y * intoLamdda);
        const outoVec = B2PolygonShape.ComputeSubmergedArea_s_outoVec.Set(this.m_vertices[outoIndex].x * (1 - outoLamdda) +
