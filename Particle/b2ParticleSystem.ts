...
function std_sort(
	...
	cmp: (a: any, b: any) => boolean = default_compare
): any[] {
	let left = first
	let stack: any[] = []
	let pos = 0
...
export class B2ParticleSystem {
	 ...
	 */
	m_handleIndexBuffer: B2ParticleSystem.UserOverridableBuffer<
		B2ParticleHandle
	> = new B2ParticleSystem.UserOverridableBuffer<B2ParticleHandle>()
	m_flagsBuffer: B2ParticleSystem.UserOverridableBuffer<
		B2ParticleFlag
	> = new B2ParticleSystem.UserOverridableBuffer<B2ParticleFlag>()
	m_positionBuffer: B2ParticleSystem.UserOverridableBuffer<
		B2Vec2
	> = new B2ParticleSystem.UserOverridableBuffer<B2Vec2>()
	m_velocityBuffer: B2ParticleSystem.UserOverridableBuffer<
		B2Vec2
	> = new B2ParticleSystem.UserOverridableBuffer<B2Vec2>()
	...
	m_colorBuffer: B2ParticleSystem.UserOverridableBuffer<
		B2Color
	> = new B2ParticleSystem.UserOverridableBuffer<B2Color>()
	m_groupBuffer: B2ParticleGroup[] = []
	m_userDataBuffer: B2ParticleSystem.UserOverridableBuffer<
		any
	...
	m_lastBodyContactStepBuffer: B2ParticleSystem.UserOverridableBuffer<
		number
	> = new B2ParticleSystem.UserOverridableBuffer<number>()
	m_bodyContactCountBuffer: B2ParticleSystem.UserOverridableBuffer<
		number
	> = new B2ParticleSystem.UserOverridableBuffer<number>()
	m_consecutiveContactStepsBuffer: B2ParticleSystem.UserOverridableBuffer<
		number
	> = new B2ParticleSystem.UserOverridableBuffer<number>()
	m_stuckParticleBuffer: B2GrowableBuffer<number> = new B2GrowableBuffer<
		number
	>(function() {
		return 0
	})
	m_proxyBuffer: B2GrowableBuffer<
		B2ParticleSystem.Proxy
	> = new B2GrowableBuffer<B2ParticleSystem.Proxy>(function() {
		return new B2ParticleSystem.Proxy()
	})
	m_contactBuffer: B2GrowableBuffer<B2ParticleContact> = new B2GrowableBuffer<
		B2ParticleContact
	>(function() {
	...
	> = new B2GrowableBuffer<B2ParticleBodyContact>(function() {
		return new B2ParticleBodyContact()
	})
	m_pairBuffer: B2GrowableBuffer<B2ParticlePair> = new B2GrowableBuffer<
		B2ParticlePair
	>(function() {
		return new B2ParticlePair()
	})
	m_triadBuffer: B2GrowableBuffer<B2ParticleTriad> = new B2GrowableBuffer<
		B2ParticleTriad
	>(function() {
	 ...
	 * seconds.
	 */
	m_expirationTimeBuffer: B2ParticleSystem.UserOverridableBuffer<
		number
	> = new B2ParticleSystem.UserOverridableBuffer<number>()
	/**
	 * List of particle indices sorted by expiration time.
	 */
	m_indexByExpirationTimeBuffer: B2ParticleSystem.UserOverridableBuffer<
		number
	> = new B2ParticleSystem.UserOverridableBuffer<number>()
	/**
	...
	DestroyParticlesInShape(
		...
		const callback = new B2ParticleSystem.DestroyParticlesInShapeCallback(
			this,
			shape,
			xf,
			callDestructionListener
	...
	JoinParticleGroups(groupA: B2ParticleGroup, groupB: B2ParticleGroup): void {
		...
		let filter = new B2ParticleSystem.JoinParticleGroupsFilter(
			groupB.m_firstIndex
		)
		this.UpdateContacts(true)
	...
	SplitParticleGroup(group: B2ParticleGroup): void {
		...
		let survivingList = B2ParticleSystem.FindLongestParticleList(
			group,
			nodeBuffer
		)
	...
	QueryAABB(callback: B2QueryCallback, aabb: B2AABB): void {
		...
		let firstProxy = std_lower_bound(
	...
	FreeUserOverridableBuffer(
		b: B2ParticleSystem.UserOverridableBuffer<any>
	): void {
	 ...
	 */
	ReallocateBuffer4(
		buffer: B2ParticleSystem.UserOverridableBuffer<any>,
		oldCapacity: number,
		newCapacity: number,
		deferred: boolean
	): any[] {
	...
	CreateParticleForGroup(
		...
		B2Vec2.AddVV(
			...
			B2Vec2.CrossSV(
				...
				B2Vec2.SubVV(particleDef.position, groupDef.position, B2Vec2.s_t0),
				B2Vec2.s_t0
			),
			particleDef.velocity
		)
	...
	UpdatePairsAndTriads(
		firstIndex: number,
		lastIndex: number,
		filter: B2ParticleSystem.ConnectionFilter
	): void {
		...
		if (particleFlags & B2ParticleSystem.k_triadFlags) {
			...
			let system = this
			let callback = function UpdateTriadsCallback(
				a: number,
				b: number,
				c: number
			): void {
	...
	static InitializeParticleLists(
		group: B2ParticleGroup,
		nodeBuffer: B2ParticleSystem.ParticleListNode[]
	): void {
	...
	static MergeParticleLists(
		listA: B2ParticleSystem.ParticleListNode,
		listB: B2ParticleSystem.ParticleListNode
	): void {
	...
	static FindLongestParticleList(
		group: B2ParticleGroup,
		nodeBuffer: B2ParticleSystem.ParticleListNode[]
	): B2ParticleSystem.ParticleListNode {
		let particleCount = group.GetParticleCount()
		/*ParticleListNode**/
	...
	MergeZombieParticleListNodes(
		group: B2ParticleGroup,
		nodeBuffer: B2ParticleSystem.ParticleListNode[],
		survivingList: B2ParticleSystem.ParticleListNode
	): void {
		let particleCount = group.GetParticleCount()
		for (let i = 0; i < particleCount; i++) {
			/*ParticleListNode**/
	...
	static MergeParticleListAndNode(
		...
		node: B2ParticleSystem.ParticleListNode
	): void {
	...
	GetInsideBoundsEnumerator(
		...
		return new B2ParticleSystem.InsideBoundsEnumerator(
			this,
			lowerTag,
			upperTag,
			firstProxy,
	...
	FindContacts_Reference(contacts: B2GrowableBuffer<B2ParticleContact>): void {
		...
		for (let a = beginProxy, c = beginProxy; a < endProxy; a++) {
			let rightTag = B2ParticleSystem.computeRelativeTag(
			...
			let bottomLeftTag = B2ParticleSystem.computeRelativeTag(
				this.m_proxyBuffer.data[a].tag,
				-1,
				1
			)
			...
			let bottomRightTag = B2ParticleSystem.computeRelativeTag(
				this.m_proxyBuffer.data[a].tag,
				1,
				1
			)
	...
	UpdateProxies_Reference(
		proxies: B2GrowableBuffer<B2ParticleSystem.Proxy>
	): void {
	...
	FilterContacts(contacts: B2GrowableBuffer<B2ParticleContact>): void {
		...
		let predicate = function(contact: B2ParticleContact): boolean {
			return (
				contact.flags & B2ParticleFlag.B2_particleContactFilterParticle &&
	...
	NotifyContactListenerPreContact(
		particlePairs: B2ParticleSystem.B2ParticlePairSet
	): void {
	...
	static B2ParticleContactIsZombie(contact: B2ParticleContact): boolean {
		return (
			(contact.flags & B2ParticleFlag.B2_zombieParticle) ===
	...
	NotifyBodyContactListenerPreContact(
		fixtureSet: B2ParticleSystem.FixtureParticleSet
	): void {
	...
	UpdateBodyContacts(): void {
		...
		let callback = new B2ParticleSystem.UpdateBodyContactsCallback(
			this,
	...
	SolveStaticPressure(step: B2TimeStep): void {
		...
		for (let t = 0; t < this.m_def.staticPressureIterations; t++) {
			...
			for (let i = 0; i < this.m_count; i++) {
				...
				if (
					this.m_flagsBuffer.data[i] & B2ParticleFlag.B2_staticPressureParticle
				) {
	...
	SolveZombie(): void {
		...
		for (let group = this.m_groupList; group; group = group.GetNext()) {
			...
			if (firstIndex < lastIndex) {
				...
				group.m_lastIndex = 0
				if (
					!(group.m_groupFlags & B2ParticleGroupFlag.B2_particleGroupCanBeEmpty)
				) {
					this.SetGroupFlags(
						group,
						group.m_groupFlags |
	...
	SolveLifetimes(step: B2TimeStep): void {
		...
		if (this.m_expirationTimeBufferRequiresSorting) {
			 ...
			 */
			let ExpirationTimeComparator = function(
	...
	GetParticleContactListener(): B2ContactListener {
		return this.m_allParticleFlags &
			B2ParticleFlag.B2_particleContactListenerParticle
			? this.m_world.m_contactManager.m_contactListener
			: null
	}
	...
	RemoveSpuriousBodyContacts(): void {
		...
		let discarded = 0
		let B2ParticleBodyContactRemovePredicate = function(
			contact: B2ParticleBodyContact
		): boolean {
			...
			if (!contact.fixture.TestPoint(pos)) {
				...
				for (let childIndex = 0; childIndex < childCount; childIndex++) {
					let normal = s_normal
					let distance = contact.fixture.ComputeDistance(
						pos,
						normal,
						childIndex
					)
	...
	IsRigidGroup(group: B2ParticleGroup): boolean {
		return (
			group !== null &&
			(group.m_groupFlags & B2ParticleGroupFlag.B2_rigidParticleGroup) !== 0
		)
	}
...
export namespace B2ParticleSystem {
	...
	export class FixtureParticleSet extends B2ParticleSystem.FixedSetAllocator {
		Initialize(
			...
			flagsBuffer: B2ParticleSystem.UserOverridableBuffer<B2ParticleFlag>
		): void {
			// TODO
		}
	...
	export class B2ParticlePairSet extends B2ParticleSystem.FixedSetAllocator {
		Initialize(
			...
			flagsBuffer: UserOverridableBuffer<B2ParticleFlag>
		): void {
			// TODO
		}
		...
		IsNecessary(index: number): boolean {
			return (
				(this.m_flagsBuffer.data[index] &
					B2ParticleFlag.B2_reactiveParticle) !==
				0
			)
		}
	}
	...
	export class UpdateBodyContactsCallback extends B2FixtureParticleQueryCallback {
		...
		ShouldCollideFixtureParticle(
			...
			if (this.m_contactFilter) {
				...
				if (
					flags[particleIndex] & B2ParticleFlag.B2_fixtureContactFilterParticle
				) {
		...
		ReportFixtureAndParticle(
			...
			let s_n =
				B2ParticleSystem.UpdateBodyContactsCallback.ReportFixtureAndParticle_s_n
			let s_rp =
				B2ParticleSystem.UpdateBodyContactsCallback
			...
			if (
				...
				let contact = this.m_system.m_bodyContactBuffer.data[
	...
	export class SolveCollisionCallback extends B2FixtureParticleQueryCallback {
		...
		ReportFixtureAndParticle(
			...
			let s_p1 =
				B2ParticleSystem.SolveCollisionCallback.ReportFixtureAndParticle_s_p1
			let s_output =
				B2ParticleSystem.SolveCollisionCallback
					.ReportFixtureAndParticle_s_output
			let s_input =
				B2ParticleSystem.SolveCollisionCallback.ReportFixtureAndParticle_s_input
			let s_p =
				B2ParticleSystem.SolveCollisionCallback.ReportFixtureAndParticle_s_p
			let s_v =
				B2ParticleSystem.SolveCollisionCallback.ReportFixtureAndParticle_s_v
			let s_f =
				B2ParticleSystem.SolveCollisionCallback.ReportFixtureAndParticle_s_f
