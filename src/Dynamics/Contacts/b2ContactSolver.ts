...
export class B2ContactVelocityConstraint {
	public points: B2VelocityConstraintPoint[] = B2VelocityConstraintPoint.MakeArray(
		B2_maxManifoldPoints
	)
...
export class B2ContactSolver {
	...
	public m_positionConstraints: B2ContactPositionConstraint[] = B2ContactPositionConstraint.MakeArray(
		1024
	) // TODO: B2Settings
	public m_velocityConstraints: B2ContactVelocityConstraint[] = B2ContactVelocityConstraint.MakeArray(
		1024
	) // TODO: B2Settings
	...
	public InitializeVelocityConstraints(): void {
		...
		const worldManifold: B2WorldManifold =
		...
		for (let i: number = 0; i < this.m_count; ++i) {
			...
			const manifold: B2Manifold = this.m_contacts[
				vc.contactIndex
			].GetManifold()
	...
	public SolveVelocityConstraints(): void {
		...
		for (let i: number = 0; i < this.m_count; ++i) {
			...
			for (let j: number = 0; j < pointCount; ++j) {
				...
				const newImpulse: number = B2Clamp(
	...
	public SolvePositionConstraints(): boolean {
		...
		const psm: B2PositionSolverManifold =
	...
	public SolveTOIPositionConstraints(
		toiIndexA: number,
		toiIndexB: number
	): boolean {
