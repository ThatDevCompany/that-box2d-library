...
export class B2VoronoiDiagram {
	...
	Generate(radius: number, margin: number): void {
		...
		///     m_diagram[i] = NULL;
		///   }
		this.m_diagram = B2MakeArray(this.m_countX * this.m_countY, function(
			index
		) {
			return null
		})
		...
		let queue = new B2StackQueue<B2VoronoiDiagram.Task>(
		...
		while (!queue.Empty()) {
			...
			if (a !== b) {
				...
				if (a2 > B2) {
					...
					if (y > 0) {
						queue.Push(
							new B2VoronoiDiagram.Task(x, y - 1, i - this.m_countX, b)
						)
					}
					if (x < this.m_countX - 1) {
						queue.Push(new B2VoronoiDiagram.Task(x + 1, y, i + 1, b))
